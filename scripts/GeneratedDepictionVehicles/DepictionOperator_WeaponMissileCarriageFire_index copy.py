...
# hypothesis: the X in `weapon_effet_tagX` and `WeaponShootData_0_x` is the same for any given DepictionOperator_WeaponInstantFire
def isrelevant(op: CellValue) -> bool:
    return isinstance(op, Object) and op.type == 'DepictionOperator_WeaponMissileCarriageFire'

def hypothesis(op: Object) -> bool:
    values: list[int] = []
    values.append(int(op.by_member('FireEffectTag').value[len('"weapon_effet_tag'):-1]))
    for anchor in op.by_member('WeaponShootDataPropertyName').value:
        values.append(int(anchor.value[len('"WeaponShootData_0_'):-1]))
    values.append(int(op.by_member('WeaponIndex').value))
