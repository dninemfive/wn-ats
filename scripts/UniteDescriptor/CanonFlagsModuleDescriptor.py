...
def has_str(modules_descriptors: List, module: str) -> bool:
    try:
        modules_descriptors.find_by_cond(lambda x: isinstance(x.value, str) and x.value == module).value
        return True
    except:
        return False
...
with mod.edit('GameData/Generated/Gameplay/Gfx/UniteDescriptor.ndf') as file:
    ...
    for row in file:
        ...
        if has_str(modules_descriptors, 'CanonFlagsModuleDescriptor'):
