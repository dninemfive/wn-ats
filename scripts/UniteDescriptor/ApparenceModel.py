...
with mod.edit('GameData/Generated/Gameplay/Gfx/UniteDescriptor.ndf') as file:
    ...
    for row in file:
        ...
        apparence_model: Object = modules_descriptors.by_name('ApparenceModel').value
        for member in apparence_model:
            subtypes[apparence_model.type].add(member.member)
...
with open(os.path.join(FOLDER, f'ApparenceModel.data'), 'w') as file:
    file.write('\n'.join(lines))
print(time_since(program_start))